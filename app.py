...
def init_db():
    ...
    # Haddii fees table-kii hore uu class_name lahayn,
    # waxaan si automatic ah ugu daraynaa.
    columns = db.execute(
    ...
    if "class_name" not in column_names:
        db.execute(
            "ALTER TABLE fees ADD COLUMN class_name TEXT DEFAULT ''"
        )
    ...
    # Fee-kii hore class-ka uga soo qaad student-ka
    db.execute("""
        UPDATE fees
        SET class_name = (
            SELECT class_name
            FROM students
            WHERE students.name = fees.student_name
            LIMIT 1
        )
...
def results():
    ...
    selected_class = request.values.get(
        "class_name",
        ""
    )
